/**
 * Created by 1106886 on 10/11/14.
 */
public class StandardDeviation {
    public static Image getIsotropicStandardDeviation(Image i, int sigma){
        ...
        for(int y = sigma*3; y <paddedImage.height - sigma*3; y++){
            for(int x = sigma*3; x < paddedImage.width - sigma*3; x++){
                double xbar = getIsotropicWeightedAverage(paddedImage, sigma, x, y);
    ...
    private static double getIsotropicWeightedAverage(Image i, int sigma, int px, int py){
        double runningTotal = 0;

        for(int y = -3*sigma; y < 3*sigma; y++) {
            for (int x = -3 * sigma; x < 3 * sigma; x++) {
                double dist = Math.sqrt(Math.pow(y, 2) + Math.pow(x, 2));
                double normalWeighting = KernelGenerator.gaussianFunction(dist, sigma);
...
//                System.out.println("x-bar: "+xbar);
                double runningTotal = 0;
                double runningWeightingTotal = 0;

                for(int yk = -3 * sigma; yk < 3 * sigma; yk++){
                    int xi = paddedImage.pixels[x][y+yk];
                    double gaussianWeighting = KernelGenerator.gaussianFunction(yk, sigma);
                    runningTotal += gaussianWeighting * Math.pow((xi - xbar), 2);
                    runningWeightingTotal += gaussianWeighting;
                }
//                System.out.println("weightings total: "+runningWeightingTotal);
                sdImage.pixels[x][y-(sigma*3)] = (int) (Math.sqrt(runningTotal)/runningWeightingTotal);
            }
        }
        return sdImage;
    }
